...
<head>
    ...
    <meta property="og:image" content="{{ $ogImage ?? asset('frontend/images/hero-home.webp') }}">
...
<body class="{{ $bodyClass ?? '' }}">
    ...
    <div id="main-content" tabindex="-1" @class(['managed-content' => !str_starts_with($contentView, 'frontend.pages.')])>
        @if (session('success'))
    ...
    @include('partials.frontend.social-channels')
    @include('partials.frontend.dialogs')
    @include('partials.frontend.detail-dialog')
    @if (!str_starts_with($contentView, 'frontend.pages.'))
        ...
        @endforeach
    @endif
    <script type="application/json" id="frontend-settings">{!! json_encode($frontendSettings, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>
