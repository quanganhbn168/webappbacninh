...
<div aria-live="polite" class="theme-grid" id="themeList"><?php foreach ($themes as $theme): ?><?php view('components.theme-card', compact('theme')); ?><?php endforeach; ?></div>
...
<form class="theme-contact__form needs-validation" id="themeForm" novalidate>
<div class="row g-3">
<div class="col-md-6"><label class="form-label" for="themeName">Họ và tên *</label><input class="form-control" id="themeName" required=""/></div>
<div class="col-md-6"><label class="form-label" for="themePhone">Số điện thoại *</label><input class="form-control" id="themePhone" required="" type="tel"/></div>
<div class="col-md-6"><label class="form-label" for="themeCode">Mã giao diện</label><input class="form-control" id="themeCode" placeholder="Ví dụ: WABN-001"/></div>
<div class="col-md-6"><label class="form-label" for="themeIndustry">Lĩnh vực</label><input class="form-control" id="themeIndustry" placeholder="Sản xuất, nội thất, giáo dục..."/></div>
<div class="col-12"><label class="form-label" for="themeMessage">Nhu cầu bổ sung</label><textarea class="form-control" id="themeMessage" placeholder="Mô tả màu sắc, trang cần có hoặc website tham khảo..." rows="3"></textarea></div>
...
<script type="application/json" id="themesJson"><?= json_encode(array_map(static function (array $theme): array {
  return [
    ...
    'tags' => $theme['tags'],
    'imageUrl' => asset('assets/images/' . $theme['image']),
